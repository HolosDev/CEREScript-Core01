...
c_set01 = do
  result <- interpretCERES (SetValue (VP 1 AtWorld voidHere) i2)
                           (initialEnv, emptyEnv)
  result @?= (env_set01, emptyEnv)
c_set02 = do
  result <- interpretCERES (SetValue (VP 1 AtWorld voidHere) d1)
                           (initialEnv, emptyEnv)
  result @?= (env_set02, emptyEnv)
c_set03 = do
  result <- interpretCERES (SetValue (VP 1 AtWorld voidHere) s1)
                           (initialEnv, emptyEnv)
  result @?= (env_set03, emptyEnv)

c_modify01 = do
  result <- interpretCERES
    (ModifyValue (VP 2 AtWorld voidHere) (COAMulWith (IntValue 4)))
    (initialEnv, emptyEnv)
